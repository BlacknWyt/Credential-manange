...
router.post("/login", (req, res) => {
  ...
  let allCred = JSON.parse(fs.readFileSync("Creds/userCreds.json"));
  ...
  for (let a = 0; a < allCred.fetchedCred.length; a++) {
    if (
      allCred.fetchedCred[a].Username === username &&
      allCred.fetchedCred[a].Password === password
    ) {
  ...
  if (pass) {
    ...
    //if user doesnt exist then this is the response
...
router.post("/Register", (req, res) => {
  ...
  userCon.divCreate(username, name, email, ou, div, password);
...
router.post("/credentials", (req, res) => {
  ...
  credCon.create(username, password, site, ou, div);
  res.json({ success: true });
});
